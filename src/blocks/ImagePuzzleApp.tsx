...
interface ImagePuzzleUploadProps {
  ...
  setimagePos: (imagePos: number[]) => void;
  image: File | null;
  imagePos: number[]
  puzzleSize: number;
  zoomLevel: number;
...
function ImagePuzzleUpload(props: ImagePuzzleUploadProps) {
  ...
  useEffect(() => {
    ...
    const [sx, sy, sw, sh] = [
      props.imagePos[0] + image.width / 2 - canvas.width / 2 / props.zoomLevel,
      props.imagePos[1] + image.height / 2 - canvas.height / 2 / props.zoomLevel,
      canvas.width / props.zoomLevel,
      canvas.height / props.zoomLevel
    ];
  ...
  }, [props.imagePos, props.zoomLevel, image]);
  ...
  return (
    <div className='flex flex-col flex-0 mx-10 mt-4 aspect-square'>
      ...
      <input type='range' min='1' max='4.0' step='0.25' value={props.zoomLevel} onChange={e => props.setZoomLevel(Number(e.target.value))} />
    </div>
    ...
    <>
      ...
      >
        <ImagePuzzleUpload image={image} puzzleSize={puzzleSize} onImageUploaded={(value: File | null) => setImage(value)}
          imagePos={imagePos} setimagePos={setimagePos} zoomLevel={zoomLevel} setZoomLevel={setZoomLevel} />
