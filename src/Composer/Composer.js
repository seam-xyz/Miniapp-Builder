...
function Composer({ addNewPost }) {
  ...
  let supportedBlocks = Object.entries(BlockTypes).filter(
    (blockType) => !blockType[1].deprecated && blockType[1].doesBlockPost
  ).reverse();
  ...
  const canDismiss = async (data, role) => {
    return role !== 'gesture';
  };
    ...
    <div className={`${classes.noScrollBar} mx-4 my-0`}>
      ...
      <IonList>
        {supportedBlocks.map((block) => (
          <div key={block[1].type}>
            <ComposerMiniAppItem
              block={block[1]}
              tapAction={() => chooseBlock(block[1])}
            />
            <Divider />
          </div>
        ))}
  ...
  const editBlockStep = () => (
    <div style={{ maxHeight: '100%', display: 'flex', flexDirection: 'column', width: '100%', height: '100%' }}>
      <div style={{ position: BlockFactory.doesBlockEditFullscreen(state.selectedBlockType?.type) ? 'absolute' : 'relative', paddingBottom: 16, width: '100%' }}>
        <SeamHeaderBar
  ...
  const renderBlockPreview = (blockData) => {
    ...
    return (
      <div className='flex flex-row justify-center min-w-full min-h-full' ref={blockPreviewRef}>
        {blockPreview}
  ...
  const previewBlockStep = () => (
    <div className='flex flex-col h-full my-0'>
      ...
      <div className='flex-grow overflow-auto'>
        {state.selectedBlockData != {} && renderBlockPreview(state.selectedBlockData)}
      </div>
      <div className='p-4 bg-[#FCFCFC] rounded-[20px] border-2 border-b-0 border-seam-gray mb-4 w-full' style={{ marginLeft: '-1rem', marginRight: '-1rem', paddingBottom: 'env(safe-area-inset-bottom)' }}>
        <div className='flex flex-row w-full'>
          <div className='mr-4'>
            <img className='rounded-full h-8 w-8' src='https://upcdn.io/W142hWW/raw/uploads/2024/02/23/start-4skb.png' alt='Profile' />
          </div>
          <div className='w-full'>
            <Input
              textValue={state.description}
              onChange={(_, newValue) => dispatch({ type: 'setDescription', payload: newValue })}
        ...
        <Button onClick={handleCreatePost} fullWidth variant='contained' color='primary' className='mt-4 h-[60px] justify-center items-center flex rounded-[8px] bg-seam-blue'>
  ...
  return (
    <div>
      <div className='py-2 px-6 cursor-pointer' onClick={() => dispatch({ type: 'setIsOpen', payload: true })} id='open-modal' expand='block'>
        <Plus color='white' />
      </div>
      <IonModal
